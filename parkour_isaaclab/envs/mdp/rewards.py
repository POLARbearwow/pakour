...
class reward_feet_edge(ManagerTermBase):
    def __init__(self, cfg: RewardTermCfg, env: ParkourManagerBasedRLEnv):
        ...
        self.parkour_event: ParkourEvent = env.parkour_manager.get_term(
            cfg.params["parkour_name"]
        )
        ...
        total_x_edge_maskes = torch.from_numpy(
            self.parkour_event.terrain.terrain_generator_class.x_edge_maskes
    ...
    def __call__(
        ...
        feet_pos_x = (
            (
                (
                    self.asset.data.body_state_w[:, self.asset_cfg.body_ids, 0]
                    + self.rows_offset
                )
        ...
        feet_pos_y = (
            (
                (
                    self.asset.data.body_state_w[:, self.asset_cfg.body_ids, 1]
                    + self.cols_offset
                )
                / self.horizontal_scale
            )
        ...
        rew = (self.parkour_event.terrain.terrain_levels > 3) * torch.sum(
...
class reward_action_rate(ManagerTermBase):
    def __init__(self, cfg: RewardTermCfg, env: ParkourManagerBasedRLEnv):
        ...
        self.previous_actions = torch.zeros(
            env.num_envs, 2, asset.num_joints, dtype=torch.float, device=self.device
        )
    ...
    def __call__(
        ...
        self.previous_actions[:, 1, :] = env.action_manager.get_term(
            "joint_pos"
        ).raw_actions
        return torch.norm(
            self.previous_actions[:, 1, :] - self.previous_actions[:, 0, :], dim=1
        )
...
class reward_dof_acc(ManagerTermBase):
    def __init__(self, cfg: RewardTermCfg, env: ParkourManagerBasedRLEnv):
        super().__init__(cfg, env)
        asset: Articulation = env.scene[cfg.params["asset_cfg"].name]
        self.previous_joint_vel = torch.zeros(
            env.num_envs, 2, asset.num_joints, dtype=torch.float, device=self.device
        )
    ...
    def __call__(
        ...
        return torch.sum(
            torch.square(
                (self.previous_joint_vel[:, 1, :] - self.previous_joint_vel[:, 0, :])
                / self.dt
            ),
            dim=1,
        )
...
def reward_tracking_goal_vel(
    ...
    target_vel = target_pos_rel / (
        torch.norm(target_pos_rel, dim=-1, keepdim=True) + 1e-5
    )
...
class reward_delta_torques(ManagerTermBase):
    ...
    def __call__(
        ...
        return torch.sum(
            torch.square(
                (self.previous_torque[:, 1, :] - self.previous_torque[:, 0, :])
            ),
            dim=1,
        )
